(()=>{
  ...
  function localPortraitPoint(clientX,clientY){
    const s=stage();
    if(!s)return null;
    const rect=s.getBoundingClientRect();
    if(!rect.width||!rect.height)return null;
    return {
      x:(clientX-rect.left)*PORTRAIT_W/rect.width,
      y:(clientY-rect.top)*PORTRAIT_H/rect.height
    };
  }

  function portraitBoothHit(clientX,clientY){
    if(!boothMask)return false;
    const point=localPortraitPoint(clientX,clientY);
  ...
  function cleanBoothHit(event){
    ...
    return event.target===cleanBooth || event.composedPath?.().includes(cleanBooth)===true;
  }
  ...
  function installCleanBooth(){
    ...
    if(current.dataset.mqV9Clean==='1'){
      cleanBooth=current;
      return true;
    }
  ...
  function sanitizeStage(){
    const s=stage();
    if(!s||!cleanBooth)return;
    removeLegacyRuntimeState();
    for(const node of s.children){
      if(node===cleanBooth)continue;
      node.style.setProperty('pointer-events','none','important');
      if(node instanceof HTMLElement)node.style.setProperty('cursor','default','important');
    }
    cleanBooth.style.setProperty('pointer-events',portraitMql.matches?'none':'auto','important');
    cleanBooth.style.setProperty('cursor','none','important');
  }

  function triggerOriginalBooth(){
    if(syntheticClick||!originalBooth||!isExteriorInteractive())return;
    syntheticClick=true;
    clearHover();
    try{originalBooth.click()}finally{queueMicrotask(()=>{syntheticClick=false})}
  }
  ...
  function start(){
    ...
    document.body&&new MutationObserver(()=>{
