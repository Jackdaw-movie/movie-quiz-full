(()=>{
  ...
  const VERSION='responsive-exterior-v9.5-glow-mask-single-source';
  ...
  let portraitBoothAlpha=null;
  let portraitBoothAlphaW=0;
  let portraitBoothAlphaH=0;
  let portraitGlowAlpha=null;
  let portraitGlowAlphaW=0;
  let portraitGlowAlphaH=0;
  ...
  async function loadInteractionMasks(){
    const [portraitBooth,portraitGlow,desktopBooth]=await Promise.all([
      portraitBoothAlpha?Promise.resolve(null):loadAlpha(PORTRAIT_BOOTH_SRC),
      portraitGlowAlpha?Promise.resolve(null):loadAlpha(PORTRAIT_GLOW_SRC),
      desktopBoothAlpha?Promise.resolve(null):loadAlpha(DESKTOP_BOOTH_SRC)
    ]);
    if(portraitBooth){
      portraitBoothAlpha=portraitBooth.alpha;
      portraitBoothAlphaW=portraitBooth.width;
      portraitBoothAlphaH=portraitBooth.height;
    }
    if(portraitGlow){
      portraitGlowAlpha=portraitGlow.alpha;
      portraitGlowAlphaW=portraitGlow.width;
      portraitGlowAlphaH=portraitGlow.height;
    }
    if(desktopBooth){
      desktopBoothAlpha=desktopBooth.alpha;
      desktopBoothAlphaW=desktopBooth.width;
      desktopBoothAlphaH=desktopBooth.height;
    }
  }
  ...
  function sampleAlpha(alpha,width,height,x,y,threshold){
    if(!alpha||!width||!height)return false;
  ...
  function portraitBoothHit(clientX,clientY){
    ...
    const boothHit=sampleAlpha(
      portraitBoothAlpha,portraitBoothAlphaW,portraitBoothAlphaH,
      point.x-PORTRAIT_BOOTH_X,point.y-PORTRAIT_BOOTH_Y,32
    );
    const glowHit=sampleAlpha(
      portraitGlowAlpha,portraitGlowAlphaW,portraitGlowAlphaH,
      point.x-PORTRAIT_GLOW_X,point.y-PORTRAIT_GLOW_Y,24
    );
    return boothHit||glowHit;
  }

  function desktopBoothHit(clientX,clientY){
    if(!desktopBoothAlpha)return false;
    ...
    return sampleAlpha(
      desktopBoothAlpha,desktopBoothAlphaW,desktopBoothAlphaH,
      nx*(desktopBoothAlphaW-1),ny*(desktopBoothAlphaH-1),32
    );
  }
  ...
  function installCleanBooth(){
    ...
    if(current.dataset.mqV95Clean==='1'){
      cleanBooth=current;
      return true;
    }
    detachedOriginalBooth=current;
    cleanBooth=current.cloneNode(true);
    cleanBooth.dataset.mqV95Clean='1';
  ...
  function sanitizeStage(){
    ...
    const hoverActive=root().classList.contains('mq-v9-booth-hover');
    for(const node of s.children){
      node.style.setProperty('pointer-events','none','important');
      if(node instanceof HTMLElement)node.style.setProperty('cursor',hoverActive?'none':'default','important');
    }
    if(cleanBooth){
