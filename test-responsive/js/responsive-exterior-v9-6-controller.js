(()=>{
  ...
  const VERSION='responsive-exterior-v9.6-filled-glow-hit';
  ...
  const PORTRAIT_GLOW_X=455;
  const PORTRAIT_GLOW_Y=650;
  ...
  const PORTRAIT_BOOTH_SRC=ASSET_BASE+'booth.png';
  const PORTRAIT_GLOW_SRC=ASSET_BASE+'booth-glow-only.png';
  ...
  let portraitHitMask=null;
  let portraitHitW=0;
  let portraitHitH=0;
  let portraitHitX=0;
  let portraitHitY=0;
  let desktopHitMask=null;
  let desktopHitW=0;
  let desktopHitH=0;
  ...
  function buildFilledRowMask(layers,padding=0){
    const valid=layers.filter(layer=>layer&&layer.alpha&&layer.width&&layer.height);
    if(!valid.length)return null;
    const minX=Math.min(...valid.map(layer=>layer.x));
    const minY=Math.min(...valid.map(layer=>layer.y));
    const maxX=Math.max(...valid.map(layer=>layer.x+layer.width));
    const maxY=Math.max(...valid.map(layer=>layer.y+layer.height));
    const width=maxX-minX;
    const height=maxY-minY;
    const raw=new Uint8Array(width*height);

    for(const layer of valid){
      const ox=layer.x-minX;
      const oy=layer.y-minY;
      const threshold=layer.threshold??24;
      for(let y=0;y<layer.height;y++){
        const srcRow=y*layer.width;
        const dstRow=(oy+y)*width+ox;
        for(let x=0;x<layer.width;x++){
          if(layer.alpha[srcRow+x]>threshold)raw[dstRow+x]=1;
        }
      }
    }
    ...
    for(let y=0;y<height;y++){
      ...
      for(let x=0;x<width;x++){
        if(raw[row+x]){left=x;break;}
      }
      if(left<0)continue;
      for(let x=width-1;x>=0;x--){
        if(raw[row+x]){right=x;break;}
      }
    ...
    return {mask:filled,width,height,x:minX,y:minY};
  }

  async function loadInteractionMasks(){
    const [portraitBooth,portraitGlow,desktopBooth]=await Promise.all([
      loadAlpha(PORTRAIT_BOOTH_SRC),
      loadAlpha(PORTRAIT_GLOW_SRC),
      loadAlpha(DESKTOP_BOOTH_SRC)
    ]);

    const portrait=buildFilledRowMask([
      portraitBooth&&{...portraitBooth,x:PORTRAIT_BOOTH_X,y:PORTRAIT_BOOTH_Y,threshold:24},
      portraitGlow&&{...portraitGlow,x:PORTRAIT_GLOW_X,y:PORTRAIT_GLOW_Y,threshold:10}
    ],0);
    if(portrait){
      portraitHitMask=portrait.mask;
      portraitHitW=portrait.width;
      portraitHitH=portrait.height;
      portraitHitX=portrait.x;
      portraitHitY=portrait.y;
    }

    if(desktopBooth){
      const desktop=buildFilledRowMask([
        {...desktopBooth,x:0,y:0,threshold:18}
      ],2);
      if(desktop){
        desktopHitMask=desktop.mask;
        desktopHitW=desktop.width;
        desktopHitH=desktop.height;
      }
    }
  }
  ...
  function portraitBoothHit(clientX,clientY){
    if(!portraitHitMask)return false;
    const point=stageLocalPoint(clientX,clientY,PORTRAIT_W,PORTRAIT_H);
    if(!point)return false;
    return sampleMask(
      portraitHitMask,portraitHitW,portraitHitH,
      point.x-portraitHitX,point.y-portraitHitY
    );
  }

  function desktopBoothHit(clientX,clientY){
    if(!desktopHitMask)return false;
    ...
    return sampleMask(
      desktopHitMask,desktopHitW,desktopHitH,
      nx*(desktopHitW-1),ny*(desktopHitH-1)
    );
  }
  ...
  function installCleanBooth(){
    ...
    if(current.dataset.mqV96Clean==='1'){
      cleanBooth=current;
      return true;
    }
    detachedOriginalBooth=current;
    cleanBooth=current.cloneNode(true);
    cleanBooth.dataset.mqV96Clean='1';
  ...
  function captureExterior(event){
    ...
    if(isProtectedExteriorUi(event)){
      if(event.type==='pointermove'||event.type==='mousemove')clearHover();
      return;
    }
